        ...
        if ($httpCode !== $successCode) {
            ...
            if (!empty($response['error_description'])) {
                ...
                $errorMessage = $response['requestError']['serviceException']['text']
                    . ' ' . (is_array($response['requestError']['serviceException']['variables']) && !empty($response['requestError']['serviceException']['variables']) ? $response['requestError']['serviceException']['variables'][0] : $response['requestError']['serviceException']['variables']);
            } elseif (!empty($response['requestError']['policyException'])) {
                $errorMessage = $response['requestError']['policyException']['text']
                    . ' ' . (is_array($response['requestError']['policyException']['variables']) && !empty($response['requestError']['policyException']['variables']) ? $response['requestError']['policyException']['variables'][0] : $response['requestError']['policyException']['variables']);
            }
